...
$(document).ready(function () {
    let table = $('#amc_quotation').DataTable({
        ...
        columns: [
            ...
            { data: 'quarter_id', name: 'quarter_id' },
            { data: 'status', name: 'status' },
            { data: 'contract_period_from', name: 'contract_period_from' },
            { data: 'contract_period_to', name: 'contract_period_to' },
    ...
    $(document).on('click', '.view-amc', function (e) {
        ...
        $.ajax({
            url: "{{ url('/masters/contract/amc/show/modal') }}/" + id,
            type: 'GET',
            success: function (res) {
                ...
                if (res.length > 0) {
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            <div><strong>Company Name:</strong> ${data.company_name?.company_name || '-'}</div>
                            <div><strong>AMC No / Date:</strong> ${data.amc_no || '-'} / ${data.amc_date || '-'}</div>
                        ...
                        <div class="col-md-4">
                            <div><strong>Financial Year:</strong> ${data.finance_name?.financial_year || '-'}</div>
                            <div><strong>Remark:</strong> ${data.remark || '-'}</div>
                            <div><strong>Employee Name:</strong> ${data.employee?.name || '-'}</div>
                        </div>
                        <div class="col-md-4">
                            <div><strong>Client Group:</strong> ${data.clients__group?.name || '-'}</div>
                            <div><strong>Client Name:</strong> ${data.clients_name?.client_name || '-'}</div>
                            <div><strong>Address:</strong> ${data.clients_name?.address || '-'}</div>
